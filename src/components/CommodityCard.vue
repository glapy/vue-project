<template>
  <div class="commodityCard">
    <router-link
      :to="sell ? '/shop/commodity/' + item.id : '/commodity/' + item.id"
    >
      <div class="p-img">
        <a><img :src="img" /></a>
      </div>
      <div class="p-price">
        {{ (item.price * item.discount) | currency
          ...
          ><i class="price">[{{ item.price | currency }}]</i></span
        >
      </div>
      <div class="p-name">
        <h5>{{ item.name }}</h5>
      ...
      <div class="p-commit">
        <a>{{ item.evaluationCoxunt }}+</a><strong>条评论</strong>
      </div>
      <div class="p-shop">商品货号：{{ item.shopName }}</div>
      <div class="p-degree">商品评分：{{ item.score }}</div>
    </router-link>
    <div v-if="isNeedMask" class="mask">
      <el-button v-if="sell" size="mini" @click="checkCommodity(item.id)"
        >详情</el-button
      >
      <el-button v-if="sell" size="mini" @click="updateCommodity"
        >修改</el-button
      >
      <el-button v-if="admin" size="mini" @click="auditCommodity('1')"
        >审核通过</el-button
      >
      <el-button v-if="admin" size="mini" @click="auditCommodity('2')"
        >审核失败</el-button
      >
...
export default {
  ...
  created() {
    this.img = require("../assets" + this.item.photoOne);
  },
  methods: {
    //下架商品
    deleteCommodity() {
