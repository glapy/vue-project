<template>
  <div class="userAudit">
    ...
    <el-table :data="userList" style="width: 80%">
      ...
      >
        <template slot-scope="scope">
          <el-tag :type="getTap(scope.row)" disable-transitions>{{
            scope.row.status | statusValue
          }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="审核结果">
        <template slot-scope="scope">
          <el-button size="mini" @click="handleEdit(scope.row, 'user')"
            >通过</el-button
          >
          <el-button
            size="mini"
            type="danger"
            @click="handleDelete(scope.row, 'user')"
            >失败</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-table :data="sellList" style="width: 80%">
      ...
      <el-table-column label="审核结果">
        <template slot-scope="scope">
          <el-button size="mini" @click="handleEdit(scope.row, 'sell')"
            >通过</el-button
          >
          <el-button
            size="mini"
            type="danger"
            @click="handleDelete(scope.row, 'sell')"
            >失败</el-button
          >
        </template>
      </el-table-column>
