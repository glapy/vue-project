<template>
  <div class="shop">
    <h1>
      <i class="el-icon-shopping-cart-2"></i>{{ sell.name }}的店铺<el-button
        class="addBt"
        icon="el-icon-plus"
        size="mini"
        @click="addCommodity"
    ...
    <div class="waitCheck" v-if="waitCheck != null">
      <h3><i class="el-icon-refresh"></i>待审核商品</h3>
      <CommodityList
        @update="update"
    ...
    <div class="soldOut" v-if="soldOut != null">
      <h3><i class="el-icon-sold-out"></i>已下架商品</h3>
      <CommodityList
        @update="update"
        @updateCommodity="updateCommodity"
        :list="soldOut"
    ...
    <h3><i class="el-icon-goods"></i>店内商品</h3>
    <Commodities />
  </div>
</template>
<script>
