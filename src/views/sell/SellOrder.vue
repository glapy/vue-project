  ...
  <div class="order">
    <el-table :data="orders" stripe style="width: 100%">
      ...
      <el-table-column label="操作" width="200">
        <template slot-scope="scope">
          <el-button
            v-if="determineType(scope.row.status, '发货')"
            size="mini"
            type="danger"
          ...
          <el-button
            ...
            size="mini"
            @click="handleAgreeReturn(scope.row)"
            >退货</el-button
          >
          <el-button
            v-if="determineType(scope.row.status, '拒绝退货')"
            size="mini"
            @click="handleNotAgreeReturn(scope.row)"
            >拒绝退货</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  created() {
    this.axios
      .get("/order/shop/" + this.sell.id, { params: { page: 1 } })
      .then((response) => {
