<template>
  <div class="order">
    <el-table :data="orders" stripe style="width: 100%" max-height="1000px">
      ...
      <el-table-column label="操作" width="200" fixed="right">
        <template slot-scope="scope">
          <el-button
            v-if="determineType(scope.row.status, '付款')"
            size="mini"
            class="pay"
          ...
          >
          <el-button
            v-if="determineType(scope.row.status, '取消订单')"
            size="mini"
            @click="handleCancelPay(scope.row)"
            >取消订单</el-button
          >
          <el-button
            v-if="determineType(scope.row.status, '退货')"
            class="backup"
            size="mini"
            @click="handleReturnGoods(scope.row)"
            >退货</el-button
          >
          <el-button
            v-if="determineType(scope.row.status, '确认订单')"
            size="mini"
            @click="handleConfirm(scope.row)"
            >确认订单</el-button
          >
          <el-button
            v-if="determineType(scope.row.status, '取消退货')"
            size="mini"
...
export default {
  ...
  created() {
    this.axios
      .get("/order/user/" + this.user.id, { params: { page: 1 } })
      .then((response) => {
  ...
  methods: {
    ...
    handleCancelReturn(row) {
      this.$confirm("确认取消退货? 确认后订单不能再次退货！")
        .then(() => {
    ...
    determineType(status, operation) {
      if (operation == "付款") {
        ...
        if (status == 4 || status == 3 || status == 2) return true;
