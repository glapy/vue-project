<template>
  <div class="shoppingCart">
    <el-button class="operation" size="mini" @click="handleSetCartItems"
      >清空购物车</el-button
    >
    <el-button class="operation" size="mini" @click="handleDeleteCartItem"
      >删除</el-button
    >
    <el-table
      :data="commodities"
    ...
    >
      ...
      <el-table-column label="操作" width="180">
        <template slot-scope="scope">
          <el-button size="mini" @click="handleSubtract(scope.row)"
            >-</el-button
          >
...
export default {
  ...
  created() {
    this.axios
      .get("/shoppingCart/" + this.user.id, { params: { page: 1 } })
      .then((response) => {
  ...
  methods: {
    ...
    handleDeleteCartItem() {
      if (this.selectCartItems.length == 0)
        ...
        this.axios
          .post("shoppingCart/delete", this.selectCartItems)
          .then((response) => {
            if (response.data.code == 0) {
    ...
    handleSetCartItems() {
      this.$confirm("确定要清空购物车吗？")
        .then(() => {
          this.axios
            .post("shoppingCart/delete", this.commodities)
            .then((response) => {
              if (response.data.code == 0) {
                this.setCartItems();
              }
