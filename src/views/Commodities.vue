<template>
  <div class="commodity">
    <!--商品排序-->
    <h3 v-if="!commodities.length">目前没有商品哦！</h3>
      ...
      <div class="commoidtySort">
        <div class="sort">
          ...
          <a
            href="javascript:;"
            class="bt curr"
            @click="handleGetCommodityList(0, 'price', 0)"
            ><span>综合</span><i class="el-icon-d-caret"></i
          ></a>
          <a
            href="javascript:;"
            class="bt"
            @click="handleGetCommodityList('1', 'inventory', 1)"
            ><span>销量</span><i class="el-icon-d-caret"></i
          ></a>
          <a
            href="javascript:;"
            class="bt"
            @click="handleGetCommodityList('1', 'evaluation_count', 2)"
            ><span>评论数</span><i class="el-icon-d-caret"></i
          ></a>
          <a
            href="javascript:;"
            class="bt"
            @click="handleGetCommodityList('1', 'price', 3)"
            ><span>价格</span><i class="el-icon-d-caret"></i>
            <!-- <em><i class="el-icon-caret-top"></i><i class="el-icon-caret-bottom"></i></em> -->
          </a>
        </div>
        <div class="selectPrice"></div>
        <div class="page">
    ...
    <commodityList v-if="!sell" :list="commodities" />
    <!-- 普通用户加载 -->
...
export default {
  ...
  methods: {
    getCommoditiesByHome() {
      //获取主页商品
      this.axios
        .get("/commodity", { params: { page: this.currentPage } })
        .then((response) => {
    ...
    getCommoditiesByKind(kindID) {
      //类别使用的方法
      this.axios
        .post("/commodity/searchOrder", { kind_id: [kindID] })
        .then((response) => {
    ...
    getCommoditiesBySearch(keyword) {
      //搜索框使用该方法
      this.axios
        .post("/commodity/searchOrder", { keyword: [keyword] })
        .then((response) => {
          if (response.status == 200) {
            this.commodities = response.data.data.records;
            this.total = response.data.data.total;
    ...
    getCommoditiesByShop(shopID) {
      //获取店铺商品
      this.axios
        .post("/commodity/searchOrder", { shop_id: [shopID] })
        .then((response) => {
          if (response.data.code == 0) {
            this.commodities = response.data.data.records;
            this.total = response.data.data.total;
    ...
    getCommodityList(type, key, index) {
      ...
      this.axios
        .post("/commodity/searchOrder", map)
        .then((response) => {
          if (response.status == 200) {
            this.commodities = response.data.data.records;
            this.total = response.data.data.total;
